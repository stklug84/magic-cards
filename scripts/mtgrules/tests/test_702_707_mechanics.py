...
from ..abilities import ActivatedAbility, SpellAbility, StaticAbility, \
    TargetSpec, TriggeredAbility
from ..compiler import compile_card
from ..effects import (CopySpell, Drain, GainLife, LoseLifeTargetMV,
                       Sequence, TargetControllerBasicLand,
...
class TestPoison(unittest.TestCase):
    ...
    def test_702_90b_infect_damage_is_poison(self):
        atk = creature(self.game, self.p0, name="Blighted",
                       power=3, keywords={"infect"})
    ...
    def test_702_164_toxic_adds_poison(self):
        atk = creature(self.game, self.p0, name="Rat", power=1,
                       keywords={"toxic:2"})
...
class TestCopies(unittest.TestCase):
    ...
    def test_707_10_copy_resolves_and_ceases(self):
        spell = card_in_hand(
            self.game, self.p0, name="Rite", mana_cost="{1}",
    ...
    def test_compiler_copy_template(self):
        ch = compile_card(_Ref(
            "Fork2", "Copy target instant or sorcery spell. You may "
            "choose new targets for the copy.", types=("Instant",)))
...
class TestCycling(unittest.TestCase):
    def test_702_29a_cycle_from_hand(self):
        game = make_game()
        p0 = game.players[0]
        ch = compile_card(_Ref("Slough2",
                               "Cycling {2}", types=("Land",),
                               mana_cost=""))
        ab = next(a for a in ch.abilities
...
class TestRiderTemplates(unittest.TestCase):
    def test_swords_rider(self):
        ch = compile_card(_Ref(
            "StP2", "Exile target creature. Its controller gains life "
            "equal to its power.", types=("Instant",), mana_cost="{W}"))
        sa = next(a for a in ch.abilities if isinstance(a, SpellAbility))
        parts = getattr(sa.effect, "parts", [sa.effect])
        self.assertTrue(any(isinstance(p, TargetControllerGainsPower)
                            for p in parts))
        game = make_game()
        p0, p1 = game.players
    ...
    def test_trophy_and_feed_riders_compile(self):
        ch = compile_card(_Ref(
            "Trophy2", "Destroy target permanent an opponent controls. "
            "Its controller may search their library for a basic land "
            "card, put it onto the battlefield, then shuffle.",
            types=("Instant",)))
        sa = next(a for a in ch.abilities if isinstance(a, SpellAbility))
        parts = getattr(sa.effect, "parts", [sa.effect])
        self.assertTrue(any(isinstance(p, TargetControllerBasicLand)
                            for p in parts))
        ch = compile_card(_Ref(
            "Feed2", "Destroy target creature or enchantment an opponent "
            "controls. You lose life equal to that permanent's mana "
            "value.", types=("Sorcery",)))
        sa = next(a for a in ch.abilities if isinstance(a, SpellAbility))
        parts = getattr(sa.effect, "parts", [sa.effect])
        self.assertTrue(any(isinstance(p, LoseLifeTargetMV)
                            for p in parts))

    def test_exsanguinate_merges_to_drain(self):
        ch = compile_card(_Ref(
            "Exsang2", "Each opponent loses X life. You gain life equal "
            "to the life lost this way.", types=("Sorcery",),
            mana_cost="{X}{B}{B}"))
        sa = next(a for a in ch.abilities if isinstance(a, SpellAbility))
        eff = sa.effect
...
class TestDiesWithCounterTriggers(unittest.TestCase):
    def test_village_pillagers_style(self):
        ch = compile_card(_Ref(
            "Pillager2", "Whenever a creature an opponent controls with "
            "a counter on it dies, you create a tapped Treasure token."))
        trig = next(a for a in ch.abilities
        ...
        treasures = [o for o in p0.battlefield
                     if "Treasure" in o.base.subtypes]
        self.assertEqual(len(treasures), 1)
        self.assertTrue(treasures[0].tapped)

    def test_once_each_turn_gate(self):
        ch = compile_card(_Ref(
            "Reaper2", "Whenever a creature an opponent controls with a "
