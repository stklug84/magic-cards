...
from ..abilities import TokenSpec, TREASURE
...
SOLDIER = TokenSpec(name="Soldier", power=1, toughness=1,
...
class TestReplacements(unittest.TestCase):
    ...
    def test_616_2_two_doublers_stack_multiplicatively(self):
        from ..overrides import _doubling_season, _adrix_and_nev
        _hooked(self.game, self.p0, "Doubling Season", _doubling_season)
        _hooked(self.game, self.p0, "Adrix and Nev, Twincasters",
                _adrix_and_nev)
    ...
    def test_615_prevention_no_life_gain(self):
        ...
        _hooked(self.game, self.p0, "Everlasting Torment",
    ...
    def test_academy_manufactor_one_of_each(self):
        ...
        _hooked(self.game, self.p0, "Academy Manufactor",
                _academy_manufactor)
    ...
    def test_614_enters_tapped_replacement_effectless_zone(self):
        ...
        self.game.replacements.floating.append(Replacement(
            EventType.ZONE_CHANGE, matches=matches, replace=replace))
        from ..objects import Characteristics, GameObject, Zone
        card = GameObject(Characteristics(name="Land", types={"Land"}),
                          self.p0)
        card.zone = Zone.HAND
