...
from mtgcards.database import CardDatabase                  # noqa: E402
from mtgcards.deck import Deck                              # noqa: E402
from mtgcards.deck import load_deck                         # noqa: E402
from mtgcards.stats import wilson_ci                        # noqa: E402

from .adapter import REPO, run_game                         # noqa: E402
from .policy import PROFILES, PolicyProfile, get_profile    # noqa: E402

_INT_KNOBS = {"wipe_board_deficit", "counter_value_threshold",
              "mulligan_min_lands", "mulligan_max_lands", "max_mulligans",
              "race_life"}
_BOOL_KNOBS = {"hold_reactive_mana"}


def _clone_deck(deck: Deck, suffix: str) -> Deck:
    return Deck(name=deck.name + suffix, path=deck.path,
                cards=list(deck.cards), commander=deck.commander)
...
def _pair_winrate(decks, db, prof_a, prof_b, games, seeds, turn_cap=40):
    ...
    for seed in seeds:
        for seating in (0, 1):
            ...
            profiles = [prof_a, prof_b] if seating == 0 \
                else [prof_b, prof_a]
            a_name = decks[seating].name
            for _ in range(games):
                rec = run_game(decks, db, rng, profiles=profiles,
                               turn_cap=turn_cap)
                total += 1
...
def _variant(base: PolicyProfile, assignment: dict) -> PolicyProfile:
    ...
    return dataclasses.replace(base, name=f"{base.name}[{label}]",
                               **assignment)
...
def parse_tune(specs):
    ...
    for spec in specs:
        ...
        if not values or not hasattr(PolicyProfile, knob):
            sys.exit(f"--tune expects KNOB=v1,v2 with a PolicyProfile "
                     f"field, got {spec!r}")
        parsed = []
...
def round_robin(decks, db, games, seeds, turn_cap=40):
    ...
    print(f"=== preset round-robin: {decks[0].name} vs {decks[1].name}, "
...
def main(argv=None):
    ...
    if args.tune:
        grid_tune(decks, db, args.games, seeds, args.base, args.tune,
                  turn_cap=args.turn_cap)
    else:
        round_robin(decks, db, args.games, seeds,
                    turn_cap=args.turn_cap)
