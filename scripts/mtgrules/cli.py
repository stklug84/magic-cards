...
from mtgcards.stats import Aggregator               # noqa: E402

from .adapter import REPO, report_model_coverage, run_game  # noqa: E402
from .policy import PROFILES, get_profile           # noqa: E402
...
def build_parser():
    ...
    ap.add_argument("--games", type=int, default=20,
                    help="games per seed (default 20)")
    ...
    ap.add_argument("--log-file", metavar="PATH",
                    help="write per-game records (JSONL)")
    ...
    ap.add_argument("--watch", action="store_true",
                    help="watch a single game live in the TUI "
                         "(implies --games 1; requires 'rich')")
    ap.add_argument("--viz-file", metavar="PATH",
                    help="record per-game visualization events + snapshots "
                         "(JSONL) for --replay")
    ap.add_argument("--replay", metavar="PATH",
                    help="replay a recorded --viz-file in the TUI "
                         "(deck arguments are not needed)")
    ap.add_argument("--game", type=int, default=1, metavar="N",
                    help="game number to replay from --replay file "
                         "(1-based, default 1)")
    return ap
...
def main(argv=None):
    ...
    if args.watch:
        ...
        watch_game(decks, db, seed=seeds[0], turn_cap=args.turn_cap,
                   profiles=profiles, viz_path=args.viz_file)
        return
    ...
    for seed in seeds:
        ...
        for g in range(args.games):
            ...
            rec = run_game(decks, db, rng, turn_cap=args.turn_cap,
                           profiles=profiles, recorder=recorder)
            rec["seed"] = seed
            if agg is None:
    ...
    if viz_writer is not None:
        viz_writer.close()
        print(f" viz recording written to {args.viz_file} "
